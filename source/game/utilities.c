 ...
 * Date: 18-05-2019
 * Project: Angry Birds
 * Description: this code implements a overlap of two images
 * an store the result in other the parameter is specified in
 * function header, keep in mind that the w_background is the
 * complete size of vector image that contain the background image
 * --------------------------------------------------------------------------------------*/
		...
		uint16_t color) 					//define the transparent color for image1
{
	...
	for (i = 0; i < limity; i++) {
		...
		actual_row_background = ((int) (y_background - up_h_span + i))
				* ((int) w_background);
		for (j = 0; j < limitx; j++) {
			actual_pixel_pos = actual_row_pos_base + j;
			if (j >= left_w_span && j < w + left_w_span && i >= up_h_span
					&& i < h + up_h_span) {
				if (*image1 != color) {
					image_result[actual_pixel_pos] = *image1;
				} else {
					image_result[actual_pixel_pos] =
							background[((int) (actual_row_background)
									+ (int) (x_background - left_w_span)
									+ (int) j)];
				}
				image1++;
			} else {
				image_result[actual_pixel_pos] =
						background[((int) (actual_row_background)
								+ (int) (x_background - left_w_span) + (int) j)];
			}
