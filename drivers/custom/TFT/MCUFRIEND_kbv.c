...
static uint16_t WIDTH=_WIDTH; 
static uint16_t	HEIGHT=_HEIGHT;
static uint8_t done_reset=0;
static uint8_t is8347;
static uint16_t width=_WIDTH; 
static uint16_t	height=_HEIGHT;
static uint16_t rotation=0;

#define delay LCD_Delay_us

void LCD_Delay_ms(uint16_t w)//Delay miliseconds Calibrado para busclk=20.9MHz
{
    uint16_t x;
    uint16_t y;
...
void LCD_Delay_us(uint16_t w){ //Calibrado para busclk=20.9MHz
	  uint16_t x=0;
	  uint16_t Z=w<<1;
	  while ( x++<Z ){}
}
...
void MCUFRIEND_kbv_reset(void)
{
    ...
    RESET_IDLE;
    delay(100);//400
    MCUFRIEND_kbv_WriteCmdData(0xB0, 0x0000);   //R61520 needs this to read ID
    delay(65000); //65000
}
...
uint16_t MCUFRIEND_kbv_readReg(uint16_t reg, int8_t index)
{
    uint16_t ret;
   // uint8_t lo;
    if (!done_reset)
...
{
    uint16_t h = MCUFRIEND_kbv_readReg(reg, 0);
    uint16_t l = MCUFRIEND_kbv_readReg(reg, 1);
...
{
    uint16_t h = MCUFRIEND_kbv_readReg(reg, 0);
    uint16_t m = MCUFRIEND_kbv_readReg(reg, 1);
    uint16_t l = MCUFRIEND_kbv_readReg(reg, 2);
...
uint16_t MCUFRIEND_kbv_readID(void)
{
   ...
   //uint16_t ret2;
    uint8_t msb;
    ret = MCUFRIEND_kbv_readReg(0,0); //(0)          //forces a reset() if called before begin()
    ...
    if (ret == 0x8989)          //SSD1289 is always 8989
        return 0x1289;
    ret = MCUFRIEND_kbv_readReg(0x67,0);//(0x67)        //HX8347-A
    if (ret == 0x4747)
        return 0x8347;
//#if defined(SUPPORT_1963) && USING_16BIT_BUS 
    ret = MCUFRIEND_kbv_readReg32(0xA1);      //SSD1963: [01 57 61 01]
    if (ret == 0x6101)
        return 0x1963;
...
//#endif
	ret = MCUFRIEND_kbv_readReg40(0xBF);
    ...
    if (ret == 0x1400)          //?RM68140:[xx FF 68 14 00] not tested yet
        return 0x6814;
    ret = MCUFRIEND_kbv_readReg32(0xD4);
    if (ret == 0x5310)          //NT35310: [xx 01 53 10]
        return 0x5310;
    ret = MCUFRIEND_kbv_readReg40(0xEF);      //ILI9327: [xx 02 04 93 27 FF] 
    if (ret == 0x9327)
        return 0x9327;
    uint32_t ret32 = MCUFRIEND_kbv_readReg32(0x04);
    msb = ret32 >> 16;
    ret = ret32;	
//    if (msb = 0x38 && ret == 0x8000) //unknown [xx 38 80 00] with D3 = 0x1602
    if (msb == 0x00 && ret == 0x8000) { //HX8357-D [xx 00 80 00]
        ...
        MCUFRIEND_kbv_pushCommand(0xB9, cmds, 3);
        msb = MCUFRIEND_kbv_readReg(0xD0,0);//0xD0
...
//        return 0x1526;          //subsequent begin() enables Command Access
    ...
    if (ret == 0xAC11)          //?unknown [xx 61 AC 11]
        return 0xAC11;
    ret = MCUFRIEND_kbv_readReg32(0xD3);//0xD3      //for ILI9488, 9486, 9340, 9341
    msb = ret >> 8;
...
//	if (ret2 == 0x93)
    	return ret2;
*/
	return MCUFRIEND_kbv_readReg(0,0);//0          //0154, 7783, 9320, 9325, 9335, B505, B509
}
...
int16_t MCUFRIEND_kbv_readGRAM(int16_t x, int16_t y, uint16_t * block, int16_t w, int16_t h)
{
    ...
    if (_lcd_capable & MIPI_DCS_REV1)
        _MR = 0x2E;
    MCUFRIEND_kbv_setAddrWindow(x, y, x + w - 1, y + h - 1);
    while (n > 0) {
        if (!(_lcd_capable & MIPI_DCS_REV1)) {
        	MCUFRIEND_kbv_WriteCmdData(_MC, x + col);
        	MCUFRIEND_kbv_WriteCmdData(_MP, y + row);
        }
        CS_ACTIVE;
        WriteCmd(_MR);
        setReadDir();
        ...
        while (n) {
            if (_lcd_capable & READ_24BITS) {
                ...
                if (_lcd_capable & READ_BGR)
                    ret = MCUFRIEND_kbv_color565(b, g, r);
                else
                    ret = MCUFRIEND_kbv_color565(r, g, b);
            } else {
                READ_16(ret);
    ...
    if (!(_lcd_capable & MIPI_DCS_REV1))
    	MCUFRIEND_kbv_setAddrWindow(0, 0, width - 1, height - 1);
    return 0;
}
...
//	    if (_lcd_ID	== 0x1963) WriteCmdParamN(0x13, 0, NULL);   //NORMAL mode
    ...
    // cope with 9320 variants
        ...
        case 0x0154:
          ...
          common_S6D:
            ...
            GS = (val & 0x80) ? (1 << 9) : 0;
            SS = (val & 0x40) ? (1 << 8) : 0;
            MCUFRIEND_kbv_WriteCmdData(0x01, GS | SS | 0x0028);       // set Driver Output Control
            goto common_ORG;
#endif
        case 0x5420:
        case 0x7793:
		...
		case 0xB509:
            ...
            MCUFRIEND_kbv_WriteCmdData(0x400, GS | NL);
            goto common_SS;
        default:
            _MC = 0x20, _MP = 0x21, _MW = 0x22, _SC = 0x50, _EC = 0x51, _SP = 0x52, _EP = 0x53;
            GS = (val & 0x80) ? (1 << 15) : 0;
            MCUFRIEND_kbv_WriteCmdData(0x60, GS | 0x2700);    // Gate Scan Line (0xA700)
          common_SS:
            SS = (val & 0x40) ? (1 << 8) : 0;
            MCUFRIEND_kbv_WriteCmdData(0x01, SS);     // set Driver Output Control
          common_ORG:
            ORG = (val & 0x20) ? (1 << 3) : 0;
#ifdef SUPPORT_8230
            ...
            MCUFRIEND_kbv_WriteCmdData(0x03, _lcd_madctl);    // set GRAM write direction and BGR=1.
            break;
#ifdef SUPPORT_1289
        case 0x1289:
    ...
    MCUFRIEND_kbv_setAddrWindow(0, 0, width - 1, height - 1);
    MCUFRIEND_kbv_vertScroll(0, HEIGHT, 0);   //reset scrolling after a rotation
}

void MCUFRIEND_kbv_drawPixel(int16_t x, int16_t y, uint16_t color)
{
    ...
    if (_lcd_capable & MIPI_DCS_REV1) {
    	...
    	MCUFRIEND_kbv_WriteCmdData(_MC, x);
    	MCUFRIEND_kbv_WriteCmdData(_MP, y);
    }
    MCUFRIEND_kbv_WriteCmdData(_MW, color);
}
...
void MCUFRIEND_kbv_setAddrWindow(int16_t x, int16_t y, int16_t x1, int16_t y1)
{
    ...
    if (_lcd_capable & MIPI_DCS_REV1) {
        ...
        MCUFRIEND_kbv_WriteCmdData(_SC, x);
        MCUFRIEND_kbv_WriteCmdData(_SP, y);
        MCUFRIEND_kbv_WriteCmdData(_EC, x1);
        MCUFRIEND_kbv_WriteCmdData(_EP, y1);
    }
}
...
void MCUFRIEND_kbv_fillBMP( const unsigned short *bmp_ptr){
	...
	MCUFRIEND_kbv_setAddrWindow(0, 0, width - 1, height - 1);
	    ...
	    for(j=0; j<320; j++) {
	      for(i=0; i<240; i++ ) {
	    	  write8((uint8_t)((*bmp_ptr)>>8));
	    	  write8((uint8_t)*bmp_ptr);
	    	  bmp_ptr++;
	      }
	    }
	    CS_IDLE;
...
void MCUFRIEND_kbv_pushColors1(uint8_t * block, int16_t n, uint8_t first)
{
    uint16_t color;
    uint8_t h, l;
    CS_ACTIVE;
...
void MCUFRIEND_kbv_pushColors2(const uint8_t * block, int16_t n, uint8_t first, uint8_t bigend) //costs 28 bytes
{
    uint16_t color;
    uint8_t h, l;
    CS_ACTIVE;
...
//        if (offset == 0 && rotation > 1) vsp = top + scrollines;   //make non-valid
		...
		if (is8347) { 
			...
			WriteCmdParamN(0x13, 0, 0); //WriteCmdParamN(0x13, 0, NULL);   //NORMAL i.e. disable scroll
		}
		return;
    }
    ...
    switch (_lcd_ID) {
    case 0x7783:
    	MCUFRIEND_kbv_WriteCmdData(0x61, _lcd_rev);   //!NDL, !VLE, REV
    	MCUFRIEND_kbv_WriteCmdData(0x6A, vsp);        //VL#
        break;
#ifdef SUPPORT_0139
    case 0x0139:
...
#ifdef SUPPORT_0154
    case 0x0154:
    	MCUFRIEND_kbv_WriteCmdData(0x31, sea);        //SEA
    	MCUFRIEND_kbv_WriteCmdData(0x32, top);        //SSA
    	MCUFRIEND_kbv_WriteCmdData(0x33, vsp - top);  //SST
        break;
#endif
#ifdef SUPPORT_1289
    case 0x1289:
	...
	case 0x9326:
	case 0xB509:
		MCUFRIEND_kbv_WriteCmdData(0x401, (1 << 1) | _lcd_rev);       //VLE, REV 
		MCUFRIEND_kbv_WriteCmdData(0x404, vsp);       //VL# 
        break;
    default:
        // 0x6809, 0x9320, 0x9325, 0x9335, 0xB505 can only scroll whole screen
    	MCUFRIEND_kbv_WriteCmdData(0x61, (1 << 1) | _lcd_rev);        //!NDL, VLE, REV
    	MCUFRIEND_kbv_WriteCmdData(0x6A, vsp);        //VL#
        break;
    }
}
...
void MCUFRIEND_kbv_invertDisplay(uint8_t i)
{
    ...
    if (_lcd_capable & MIPI_DCS_REV1) {
        if (is8347) {
            ...
            WriteCmdParamN(_lcd_rev ? 0x21 : 0x20, 0, 0);//WriteCmdParamN(_lcd_rev ? 0x21 : 0x20, 0, NULL);
        return;
    }
    // cope with 9320 style variants:
    switch (_lcd_ID) {
...
#endif
    case 0x0154:
    	MCUFRIEND_kbv_WriteCmdData(0x07, 0x13 | (_lcd_rev << 2));     //.kbv kludge
        break;
#ifdef SUPPORT_1289
    case 0x1289:
        _lcd_drivOut &= ~(1 << 13);
    ...
    case 0x9326:
	case 0xB509:
		MCUFRIEND_kbv_WriteCmdData(0x401, (1 << 1) | _lcd_rev);       //.kbv kludge VLE 
        break;
    default:
    	MCUFRIEND_kbv_WriteCmdData(0x61, _lcd_rev);
        break;
    }
}
...
*/
    ...
    case 0x1580:
        ...
        static const uint16_t R61580_regValues[]  = {
...
#if (defined(ILI9331) | defined(ILI9335))        
    case 0x9331:
    case 0x9335:
...
//        if (MCUFRIEND_kbv_readReg32(0xD3) == 0x0000) {        //weird DealExtreme EXTC=0 shield
...
#if (defined(SUPPORT_ILI9487) | defined(SUPPORT_ILI9488) )          
...
//      common_9488:
...
#if (defined(SUPPORT_R61505) | defined(SUPPORT_R61505V) | defined(SUPPORT_R61505w) )         
    case 0x1505:                //R61505
    case 0xB505:                //R61505V
    case 0xC505:                //R61505W
    ...
    MCUFRIEND_kbv_setRotation(0);             //PORTRAIT
    MCUFRIEND_kbv_invertDisplay(false);
